...
protocol TimerEventsProtocol {
  func onTimeElapsed(time timeArg: Int64, completion: @escaping (Result<Void, PigeonError>) -> Void)
}
class TimerEvents: TimerEventsProtocol {
  ...
  var codec: NativeCalculatorPigeonCodec {
    return NativeCalculatorPigeonCodec.shared
  }
  func onTimeElapsed(time timeArg: Int64, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.flutter_pigeon_native_integration_demo.TimerEvents.onTimeElapsed\(messageChannelSuffix)"
